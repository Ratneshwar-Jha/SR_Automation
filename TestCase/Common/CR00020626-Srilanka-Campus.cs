...
{
    class CR00020626_Srilanka_Campus
    {
        ...
        public void Campus()
        {
            try
            {
                RG_Instance.ReportStepLog("Initiator Creates Requisition With Campus Demand", "Info", false);
                bcIOA.AppLogin();
                bcCR.CreateNewRequisition();
                RG_Instance.ReportStepLog("Initiator able to Submit Requisition With Campus Demand", "Pass", false);
        ...
        public void OffCampus()
        {
            try
            {
                RG_Instance.ReportStepLog("Initiator Creates Requisition With OffCampus Demand", "Info", false);
                bcIOA.AppLogin();
                bcCR.CreateNewRequisition();
                RG_Instance.ReportStepLog("Initiator able to Submit Requisition With OffCampus Demand", "Pass", false);
                GlobalVariable.approverCount = 0;
                bcAA.MultiApprover();
            }
            catch (Exception ex)
