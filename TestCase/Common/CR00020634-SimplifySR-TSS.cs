...
{
    class CR00020634_SimplifySR_TSS
    {
        ...
        public void ProActive()
        {
            try
            {
                RG_Instance.ReportStepLog("Initiator Creates Requisition With ProActive Demand", "Info", false);
                bcIOA.AppLogin();
                bcCR.CreateNewRequisition();
                RG_Instance.ReportStepLog("Initiator able to Submit Requisition With ProActive Demand", "Pass", false);
        ...
        public void TSSFresher()
        {
            try
            {
                RG_Instance.ReportStepLog("Initiator Creates Requisition With TSS Fresher Demand", "Info", false);
                bcIOA.AppLogin();
                bcCR.CreateNewRequisition();
                RG_Instance.ReportStepLog("Initiator able to Submit Requisition With TSS Fresher Demand", "Pass", false);
                GlobalVariable.approverCount = 0;
                bcAA.MultiApprover();
            }
            catch (Exception ex)
